...
ffmpeg -y -hide_banner -loglevel info  \
    ...
    -ss "$( printf "%0.4f" "$( echo "scale=4; 1.8444 + ${diff_ss}  " | bc )" )" -i "${PLAYBACK_BETA}" \
                                                                                ...
                                                                                -filter_complex "
[2:a]
afftdn=nr=5,compensationdelay,alimiter,speechnorm,acompressor,
...
[vocals];
   ...
   [playback][vocals]amix=inputs=2:weights=0.551|0.669;
     ...
     [1:v]scale=s=848x480[v1];
       gradients=n=7:type=circular:s=848x480,scale=s=848x480[spats];
        gradients=n=6:type=spiral:s=848x480[vscope];
        [spats][vscope]overlay=alpha=0.6[spatscope];
          [1:a]avectorscope=s=848x480[frodo];
     [0:v]colorize=hue=$((RANDOM%361)):saturation=$(bc <<< "scale=2; $RANDOM/32767"):lightness=$(bc <<< "scale=2; $RANDOM/32767"),
          scale=s=848x480[v0]; 
          [spatscope]scale=s=848x480[scopy];
          [v1][scopy]hstack,scale=s=848x480[video_merge];
          [frodo][video_merge]hstack,scale=s=848x480[badcoffee];
