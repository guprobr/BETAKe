...
# Function to display progress using estimated file size
    render_display_progress() {
        ...
        while true; do
            ...
            local progress;
            progress=$(echo "scale=0; ($current_file_size * 10) / $total_size_bytes " | bc)
...
colorecho "aAjustar vol ${SRC_mic} em 90%";
 pactl set-source-volume "${SRC_mic}" 90%;
...
ffmpeg -y -hide_banner -loglevel info   \
    ...
    aresample=resampler=soxr:precision=28;
        ...
        [1:v]scale=s=640x360,colorchannelmixer=aa=0.44[v1];
       gradients=n=7:type=circular[spats];
        gradients=n=5:type=spiral:s=640x360[vscope];
          [0:v]tile=color=0xFF8C00:init_padding=3:layout=1x2[v0]; 
          [vscope][v1]hstack=inputs=2,scale=s=640x360[video_merge];
          [video_merge][spats]vstack=inputs=2,colorchannelmixer=aa=0.77,tile=layout=2x1,scale=s=1280x720[badcoffee];
